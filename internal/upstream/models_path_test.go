...
func TestModelsPathCNUnchanged(t *testing.T) {
	...
	// CN 零回归：仍走 /console/enterprises/personal/models（现状逐字）。
	if gotPath != "/console/enterprises/personal/models" {
		t.Errorf("cn FetchModels path=%q want /console/enterprises/personal/models", gotPath)
	}
}
...
func TestModelsPathGlobalV2(t *testing.T) {
	...
	// global 走 /v2 家族首选（PR #20 实测 /console 500 → /v2 200 完整模型表）。
	if gotPath != "/v2/enterprises/personal/models" {
		t.Errorf("global FetchModels path=%q want /v2/enterprises/personal/models", gotPath)
	}
}
...
func TestGlobalModelsProbeV2First(t *testing.T) {
	...
	// /v2 首选返回 200，/console 也应可达（fallback），断言只打 /v2 一次。
	var calls []string
	...
	if len(calls) != 1 || calls[0] != "/v2/enterprises/personal/models" {
		t.Fatalf("probe calls=%v want [/v2/enterprises/personal/models] (v2-first)", calls)
	}
...
func TestGlobalModelsProbeConsoleFallback(t *testing.T) {
	...
	if len(calls) != 2 ||
		calls[0] != "/v2/enterprises/personal/models" ||
		calls[1] != "/console/enterprises/personal/models" {
